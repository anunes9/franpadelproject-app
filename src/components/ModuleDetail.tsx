            ...
            {activeTab === 'content' && (
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                ...
                {module.technicalContent && (
                  <div className="mt-12 p-6 bg-p-green-light rounded-3xl border border-p-green/20">
                    <h3 className="text-p-green font-bold flex items-center gap-2 mb-4 uppercase tracking-widest text-sm">
                      <FileText className="h-4 w-4" />
                      Technical Deep Dive
                    </h3>
                    <MarkdownRenderer content={module.technicalContent} className="prose-p:text-p-blue/80" />
                  </div>
                )}
              </div>
            )}
                  ...
                  {module.exercises?.map((exercise) => (
                    <Card key={exercise.id} className="border-p-gray/50 shadow-none hover:border-p-green/50 transition-colors">
                      <div className="aspect-video bg-p-gray relative overflow-hidden group">
                        {exercise.media?.url ? (
                          <video src={exercise.media.url} className="w-full h-full object-cover" muted />
                        ) : (
